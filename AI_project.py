...
BASEY = SH * 0.8  # Base position (ground)
IMAGES = {}  # Dictionary to store game images
pygame.font.init()
WINDOW = pygame.display.set_mode((SW, SH))
Font = pygame.font.SysFont("comicsans", 16)

# Define file paths for game assets
BIRD = "imgs/bird1.png"
BG = "imgs/bg.png"
PIPE = "imgs/pipe.png"
...
FPS = 60  # Frames per second, reduced from 32 to 30 for better control


def static():
    """
    Display the static start screen of the game.
...
def game_start(generation, x, y):
    ...
    x (list): List to store generation numbers
    y (list): List to store scores
    ...
    pipeVelx = -3  # Pipe velocity, reduced from -4 to -3
    ...
    while True:
        ...
        for event in pygame.event.get():
            if event.type == QUIT:
                # Plot the learning progress before quitting
                plt.scatter(x, y)
        ...
        # Add a new pipe when the first pipe is about to touch left of screen
        if 0 < up_pipes[0]["x"] < 5:
        ...
        # Move the base
        basex1 -= 3  # Reduced from 4 to 3
        basex2 -= 3  # Reduced from 4 to 3
        if basex1 <= -IMAGES["base"].get_width():
            basex1 = basex2
            basex2 = basex1 + IMAGES["base"].get_width()

        # Move the background
        bgx1 -= 1  # Reduced from 2 to 1
        bgx2 -= 1  # Reduced from 2 to 1
...
def Collision(birdxpos, birdypos, up_pipes, bttm_pipes):
    ...
    Args:
    birdxpos (int): Bird's x position
    birdypos (int): Bird's y position
    up_pipes (list): List of upper pipes
    bttm_pipes (list): List of bottom pipes
    ...
    for pipe in up_pipes:
        ...
        if (
            birdypos < pipeHeight + pipe["y"]
            and abs(birdxpos - pipe["x"]) < IMAGES["pipe"][0].get_width()
        ):
            return True

    for pipe in bttm_pipes:
        if (
            birdypos + IMAGES["bird"].get_height() > pipe["y"]
            and abs(birdxpos - pipe["x"]) < IMAGES["pipe"][0].get_width()
        ):
            return True
    return False
...
def get_new_pipe():
    ...
    Returns:
    list: Contains dictionaries for upper and lower pipe positions
    """
    pipeHeight = IMAGES["pipe"][1].get_height()
    gap = int(SH / 4)
    y2 = int(
        gap + random.randrange(0, int(SH - IMAGES["base"].get_height() - 1.2 * gap))
    )
    pipex = int(SW + 300)
...
def ai_play(x, y):
    """
    Determine whether the bird should jump based on Q-values.

    Args:
    x (int): Discretized x position
...
def convert(birdxpos, birdypos, bttm_pipes):
    ...
    Returns:
    tuple: Discretized x and y positions
    """
...
def Q_update(x_prev, y_prev, jump, reward, x_new, y_new):
    ...
    Args:
    x_prev (int): Previous x state
    y_prev (int): Previous y state
    jump (bool): Whether the bird jumped or not
    reward (int): Reward received for the action
    x_new (int): New x state
    y_new (int): New y state
    """
    if jump:
...
if __name__ == "__main__":
    # Initialize Pygame and load game assets
    pygame.init()
